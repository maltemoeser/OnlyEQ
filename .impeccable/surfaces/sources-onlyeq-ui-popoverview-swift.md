...
Curve-first stack (surface seed 1512465f rolled candidate 7, the two-pane form; the user chose candidate 2, the stack). The plot spans the full width under the name and switch with the status pill in its corner; Bypass and Crossfeed sit as two small toggles under the axis, and the controls follow beneath and read as the signal path: output device with volume, preset with its "Use automatically on <device>" checkbox, then a hairline with Open Equalizer… and the gear. Import lives in the preset menu. The panel uses the system menu-bar material (Liquid Glass on macOS 26, popover vibrancy before). The memorable moment: pressing Bypass eases the curve onto the 0 dB line over the grey input spectrum, so hearing it flat and seeing it flat are one act.
