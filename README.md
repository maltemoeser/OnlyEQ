...
- **Adjust bar** under the band list: Bass, Treble, Tilt, and Strength sliders that sit over an imported profile without editing its bands, for warming up or brightening a correction you otherwise leave alone. Saved with the preset; Bypass, Revert, and Undo all include it.
...
- Volume boost up to 200%, automatic preamp so boosted EQ doesn't clip, a limiter as a safety net, a reference pill on the plot (keep any curve aside and click to hear it against your edit, level-matched), one-click bypass. Bypass keeps the preamp and matches loudness to the EQ'd signal, so the comparison is about tone instead of the louder side winning.
