...
- **Simpler controls.** The popover has one preset row with Bypass and Crossfeed side by side, a curve that empties to the input spectrum while bypassed, and a gear menu holding Settings, Check for Updates, and Quit. The right-click menu offers the same items. The editor's Reset-to-flat button is replaced by Revert (the undo arrow in the toolbar), which restores the saved preset, and the output-device picker lives only in the popover. The editor's band cards run low to high in frequency, so the strip reads like the graph. Settings gains a Sound tab for the limiter, crossfeed, and loudness.
