...
- **A popover that is the instrument.** The live curve spans the popover under the name and switch, with the input and output spectrum behind it; the controls below read as the signal path: output device and volume, preset with the device it is bound to, Bypass and Crossfeed. The panel uses the system menu-bar material (Liquid Glass on macOS 26). The menu-bar icon fills while the EQ shapes sound and outlines while it is off or bypassed.
