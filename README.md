...
- **A popover that is the instrument.** The live spectrum fills the popover under the name and switch, with the preset's response as a thin line over it and a vertical volume fader at its side. Bypass and Crossfeed sit right under it as small capsule toggles, then the preset with a checkbox that makes it the device's automatic preset, and the output device beneath. Import lives in the preset menu, and a hairline with Open Equalizer… closes the panel. Bypass eases the curve onto the 0 dB line rather than blanking it, and the corner pill appears only when something is off. The panel uses the system menu-bar material (Liquid Glass on macOS 26). The menu-bar icon fills while the EQ shapes sound and outlines while it is off or bypassed.
