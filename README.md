<p align="center">
  <img src="docs/screenshots/popover.png" width="620" alt="OnlyEQ menu bar popover">
</p>
...
- **A popover that is the instrument.** The live curve fills the left half at full height, with the input and output spectrum under it; the right column reads as the signal path: output device and volume, preset with the device it is bound to, Bypass and Crossfeed. The panel uses the system menu-bar material (Liquid Glass on macOS 26). The menu-bar icon fills while the EQ shapes sound and outlines while it is off or bypassed.
