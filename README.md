...
- **A popover that is the instrument.** The live curve spans the popover under the name and switch, with the input and output spectrum behind it; Bypass and Crossfeed sit right under it as small toggles, and the rows below read as the signal path: output device and volume, then the preset with a checkbox that makes it the device's automatic preset. Import lives in the preset menu, and a hairline with Open Equalizer… closes the panel. Bypass eases the curve onto the 0 dB line rather than blanking it. The panel uses the system menu-bar material (Liquid Glass on macOS 26). The menu-bar icon fills while the EQ shapes sound and outlines while it is off or bypassed.
...
- **Simpler controls.** The popover has one preset row with Bypass and Crossfeed side by side, a curve that empties to the input spectrum while bypassed, and a gear menu holding Settings, Check for Updates, and Quit. The right-click menu offers the same items. The editor's Reset-to-flat button is replaced by Revert, which restores the saved preset, and the output-device picker lives only in the popover. Settings gains a Sound tab for the limiter, crossfeed, and loudness.
