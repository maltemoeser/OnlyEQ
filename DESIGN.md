...
The editor window opens at 840 by 680 pt with a minimum of 720 by 580, so the plot keeps more than half the height above the control stack. Its unified 52 pt title bar shows the preset name as the window title, with “Edited” as the subtitle once the curve differs from the saved preset; the title is a menu (`toolbarTitleMenu`) of the presets, Import…, and Delete. The actions sit on the right as real toolbar items: Save…, Revert as an `arrow.counterclockwise` glyph, Bypass, and the gear. The verbs are words; the one glyph is the undo arrow. Below it: the graph filling the remaining height with 24 pt side margins, the reference pill in its top-right corner, then a 158 pt band list of 22 pt rows in two columns (three from 1056 pt wide), filled top to bottom then left to right and ordered low to high in frequency so it reads like the graph (the order holds still for the length of a node drag); ten bands and the Add Band row fit without scrolling, longer presets scroll vertically. Under it, a divider and the Adjust bar: Bass, Treble, Tilt, and Strength sliders in two rows with a Reset button, the simple way to tune a profile, always in view so the reason a curve leaves its handles is never hidden. Then a divider and the Preamp bar: the Preamp label, readout, and slider with the Auto checkbox on the left, and on the right, under its own "Output" label, the peak meter. Everything in the editor shares the 24 pt horizontal margin.
