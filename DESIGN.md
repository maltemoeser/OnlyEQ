...
The editor window opens at 840 by 560 pt with a minimum of 720 by 480. Its unified 52 pt title bar carries real toolbar items (preset menu, Save…, and Revert as an `arrow.counterclockwise` glyph on the left; the Bands / Adjust segmented control and Bypass in the centre; a text-only Import… and the gear on the right) and hides the window title. The verbs are words; the one glyph is the undo arrow. Below it: the Compare row with 8 pt above and below, a divider, the graph filling the remaining height with 24 pt side margins, a 158 pt band list of 22 pt rows in two columns (three from 1056 pt wide), filled top to bottom then left to right and ordered low to high in frequency so it reads like the graph (the order holds still for the length of a node drag); ten bands and the Add Band row fit without scrolling, longer presets scroll vertically. The toolbar's Bands / Adjust segmented control swaps the list for the Adjust pane of the same height: Bass, Treble, Tilt, and Strength sliders with a Reset button, the simple way to tune a profile. Then a divider and the Preamp bar. Everything in the editor shares the 24 pt horizontal margin.
...
### Band Row
- A 22 pt row, 4 pt side padding, 5 pt spacing: an 8 pt colour dot, the band number in caption semibold secondary right-aligned in 16 pt, a borderless type menu in a 72 pt column, then frequency, gain, and Q as 60, 58, and 50 pt value fields (the Q field reads "Q 1.41" so it names itself), and a tertiary delete glyph. Selected: a 5 pt rounded fill of the band colour at 18 %. 50 % opacity when the band is disabled. The last row is Add Band, a plain caption button with a `plus` glyph in secondary.

### Adjust Pane
- Same 158 pt as the band list, 12 pt vertical padding, rows 8 pt apart and centred. Each row: a 60 pt subheadline secondary label, a small slider, and a 64 pt subheadline medium monospaced readout ("+1.5 dB", "0.0 dB", "85%"). Bass and Treble are ±6 dB shelves at 105 Hz and 2.5 kHz, Tilt is ±6 dB at the ends about 1 kHz, Strength is 0 to 100 % of the bands' gain. A small bordered Reset button trails, disabled when everything is neutral. The graph keeps its handles on the preset's bands and draws the composite of what is heard.
