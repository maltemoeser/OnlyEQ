...
### Adjust Bar
- An "Adjust" subheadline secondary label, then a two-by-two grid (40 pt between columns, 8 pt between rows) of Bass and Treble over Tilt and Strength, 12 pt vertical padding so the bar breathes between its dividers. Each cell: a 56 pt subheadline secondary label, a small slider of at most 220 pt so its readout stays beside it, and a 64 pt subheadline medium monospaced readout aligned on the unit ("+1.5 dB", "0.0 dB", "85%", "Off" at zero strength). Bass and Treble are ±6 dB shelves at 105 Hz and 2.5 kHz, Tilt is ±6 dB at the ends about 1 kHz, Strength is 0 to 100 % of the bands' gain. A small bordered Reset button sits on the right margin, in line with the Output meter below, disabled when everything is neutral. Arrow keys move a slider one step (0.5 dB, 5 %). The graph keeps its handles on the preset's bands and draws the composite of what is heard.
