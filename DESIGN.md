...
### Reference Pill
- A small capsule in the plot's top-left corner, 10 pt after the +dB label: `circle` and "Compare" while empty, opening a menu of Use Current Curve as Reference and the presets. With a reference set it names it; clicking the body hears the reference in place of the edit (level-matched, the plot draws the reference with handles hidden, the band list and Adjust bar disable, and "Hearing reference" with an `ear` glyph appears top-right where "Bypassed" does), the dot fills and the capsule tints accent. The chevron adds Swap with Reference and Clear Reference. Setting, swapping, and clearing are named undo steps; hearing is a listening state like Bypass and is not.
