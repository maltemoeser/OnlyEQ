...
### Primary
- **System Accent** (`Color.accentColor`, `NSColor.controlAccentColor`): the curve stroke, the curve fill gradient (35 % to 3 %), the output spectrum bars, on-state switch tracks, the app badge, the boost slider fill, the reference pill while the reference is heard, the drop well while a file hovers over it (8 % fill, solid stroke), and the headphone-suggestion banner (9 % fill). It is the only saturated colour on a resting screen.
...
- **Caption** (`.caption`): the workhorse for captions, band-card field values, the reference pill, form-like rows in the import preview.
...
- **Value** (`.caption.monospacedDigit()`, `.subheadline.monospacedDigit()`): Fc, Gain, Q, dB readouts, percentages, latency, and any number that updates live. Negative readouts use the true minus sign (U+2212), never a hyphen.
...
The editor window opens at 840 by 680 pt with a minimum of 720 by 580, so the plot keeps more than half the height above the control stack. Its unified 52 pt title bar shows the preset name as the window title, with “Edited” as the subtitle once the curve differs from the saved preset; the title is a menu (`toolbarTitleMenu`) of the presets, Import…, and Delete. The actions sit on the right as real toolbar items: Save…, Revert as an `arrow.counterclockwise` glyph, and the gear. The verbs are words; the one glyph is the undo arrow. Below it: the graph filling the remaining height with 24 pt side margins, the Bypass and reference pills in its top-right corner, then a 158 pt band list of 22 pt rows in two columns (three from 1056 pt wide), filled top to bottom then left to right and ordered low to high in frequency so it reads like the graph (the order holds still for the length of a node drag); ten bands and the Add Band row fit without scrolling, longer presets scroll vertically. Under it, a divider and the Adjust bar: Bass, Treble, Tilt, and Strength sliders in two rows with a Reset button, the simple way to tune a profile, always in view so the reason a curve leaves its handles is never hidden. Then a divider and the Preamp bar: the Preamp label, readout, and slider with the Auto checkbox on the left, and on the right, under its own "Output" label, the peak meter. Everything in the editor shares the 24 pt horizontal margin.
...
### Buttons
- Native SwiftUI `.bordered` and `.borderedProminent`. Popover toggles and the device pull-down are `.small` text capsules (`.buttonBorderShape(.capsule)`; the pull-down is a `Menu` in `.button` style) and the footer's Open Equalizer… is a default-size text-only `.bordered` button; onboarding footers are `.large`; toolbar items and sheet footers are the default size. Toggles that behave like buttons (Bypass, Crossfeed) use `.button` toggle style; a setting that binds or unbinds (the preset's device binding) is a `.checkbox` toggle. The reference pill is a small capsule `Menu` in `.button` style whose primary action toggles hearing the reference, tinted accent while it is heard. There is no custom button.
...
### Adjust Bar
- An "Adjust" subheadline secondary label, then a two-by-two grid (40 pt between columns, 8 pt between rows) of Bass and Treble over Tilt and Strength, 12 pt vertical padding so the bar breathes between its dividers. Each cell: a 56 pt subheadline secondary label, a small slider of at most 220 pt so its readout stays beside it, and a 64 pt subheadline medium monospaced readout aligned on the unit ("+1.5 dB", "0.0 dB", "85%", "Off" at zero strength). Bass and Treble are ±6 dB shelves at 105 Hz and 2.5 kHz, Tilt is ±6 dB at the ends about 1 kHz, Strength is 0 to 150 % of the bands' gain, its readout orange above 100 % where it over-corrects. A small bordered Reset button sits on the right margin, in line with the Output meter below, disabled when everything is neutral. Arrow keys move a slider one step (0.5 dB, 5 %). The graph keeps its handles on the preset's bands and draws the composite of what is heard.

### Reference Pill
- A small capsule in the plot's top-right corner, after the Bypass capsule and any status label (the band limit, the empty hint): `circle` and "Compare" while empty, opening a menu of Use Current Curve as Reference and the presets. With a reference set it names it; clicking the body hears the reference in place of the edit (level-matched, the plot draws the reference with handles hidden, the band list and Adjust bar disable), the dot fills and the capsule tints accent; the pill itself is the only sign of the state. The chevron adds Swap with Reference and Clear Reference. Setting, swapping, and clearing are named undo steps; hearing is a listening state like Bypass and is not.
